...
func (app *App) jsHandler(w http.ResponseWriter, _ *http.Request) {
	...
	f, err := os.Open(app.indexBundle)
	if err != nil {
		log.Fatalf("failed to read file %q: %v", app.indexBundle, err.Error())
	}
	_, err = f.WriteTo(w)
	_ = f.Close()
	if err != nil {
		log.Fatalf("failed to copy %q to response: %v", app.indexBundle, err.Error())
	}
}
